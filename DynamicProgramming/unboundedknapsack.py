#Bruteforce soln.
# Time: o(2^c) where c is the capacity, because, we can use the same number many times,so that increases the height of the decision tree.
# space: o(n) where n is the number of items.

# The only difference with regular knaspack is, here, if we choose to take that index, then we dont essentially incrase the index .
...
def dfs_helper(index, profit, weight,capacity):
    ...
    if new_capacity > 0:
       p =  profit[index] + dfs_helper(index, profit, weight, new_capacity)
